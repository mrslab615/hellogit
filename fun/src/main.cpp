...
//void ComplexField(const Vector3 &R, const float &kr, CXV3 &C);
...
//void equation_twenty_seven(const CXV3 &E, const float &k0, const float &THETA,
//		const float &PHI, const Vector3 retpoint, const Vector3 &theta_h,
...
//
//				ComplexField(E_tube[n], total_path_tube[n], Ec_tube[n]);
//			} else {
...
////		}
//
//		float k0 = 2 * PI * f * pow(3e8, -1);
//		CXF j = CXF(0, 1);
//
//		for (int n = 0; n < tnum; n++) {
//
//
//			equation_twenty_seven(Ec_tube[n], k0, THETA, PHI, retpoint_tube[n],
...
//	float f = 15e9;
//	float multiply2 = 2.0;
//	CXF expvalue = exp(j * (kr) / c * f * multiply2 * PI);
...
//		Aphi = CXF(0, 0);
//	} else {
////		Phi_p = atan(abs(Ey) / abs(Ex));
////		Phi_p = atan2(abs(Ey), abs(Ex));
////		Phi_p = atan2(abs(Ex), abs(Ey));
//
//
////		cout <<"phi_local ="<< Phi_p*180/PI << endl;
////		Atheta = ( Ey * cos(Phi_p) +  Ex * cos(Phi_p)) * expvalue * dx * dy* (Ii);
//		Atheta = ( Ey ) * expvalue * dx * dy* (Ii);
//
//
////		Atheta = (Ex * cos(PHI) + Ey * sin(PHI)) * expvalue * dx * dy
////				* (Ii);
//
//
////			cout<<"Atheta= "<<Atheta<<endl;
//		Aphi = (Ex ) * expvalue * dx * dy * (Ii);
//
//
////		Aphi = (neg1 * Ey * sin(Phi_p) + Ex * sin(Phi_p)) * expvalue * dx * dy
////				* (Ii);
////		Aphi = (neg1 * Ex * sin(PHI) + Ey * cos(PHI)) * expvalue * dx * dy
////				* (Ii);
////			cout<<"Aphi= "<<Aphi<<endl;
//
//	}
...
////
////
////	Atheta = (Ex  * cos(PHI) + Ey  * sin(PHI)) * expvalue * dx* dy * (Ii);
////
////
////	Aphi = (-1 * E * phi_h * sin(PHI)+ E * theta_h * cos(PHI) ) * expvalue * dx * dy * (Ii);
////
////}
//
////void Raytube_Numbering(Vector3 *antr, Vector3 *antt) {
